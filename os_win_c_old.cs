//modified by Tim Anderson for Silverlight compatibility
//quick hack for proof of concept, not production quality!
//see http://www.itwriting.com/blog/1695-proof-of-concept-c-sqlite-running-in-silverlight.html
...
*/
    ...
    {
#if SQLITE_SILVERLIGHT
        public IsolatedStorageFileStream fs;
#else
...
#if SQLITE_SILVERLIGHT
    static bool ExistsFile(string sFilename)
    {
      ...
      static void DeleteFile(string sFilename)
    {
        using (var store = IsolatedStorageFile.GetUserStoreForApplication())
        {
...
#if SQLITE_SILVERLIGHT
      return SQLITE_OK; //sure, we got a lock, honest :-)
#else
    ...
    */
    ...
    )
    {
      ...
      IsolatedStorageFileStream fs = null;
        IsolatedStorageFile theStore = null;
...
dwFlagsAndAttributes |= FileOptions.RandomAccess; // FILE_FLAG_RANDOM_ACCESS;
      ...
      if ( isNT() )
      {
        ...
        {
#if SQLITE_SILVERLIGHT
            theStore = IsolatedStorageFile.GetUserStoreForApplication();
            fs = new System.IO.IsolatedStorage.IsolatedStorageFileStream(zConverted, dwCreationDisposition, dwDesiredAccess, dwShareMode, theStore);
#else
...
#if SQLITE_SILVERLIGHT
      if (fs == null) //(h == INVALID_HANDLE_VALUE)
...
pFile.zDeleteOnClose = zConverted;
    ...
    */
    ...
    )
    {
      ...
      using (var theStore = IsolatedStorageFile.GetUserStoreForApplication())
      {
          if (flags == SQLITE_ACCESS_EXISTS)
          {
               pResOut = theStore.FileExists(zFilename) ? 1 : 0;
              return SQLITE_OK;
          }
          //
          try
          {
            if (theStore.DirectoryExists(zFilename ))
              {
                  ...
                  try
                  {
                      IsolatedStorageFileStream fs = theStore.CreateFile(zTempFilename);
                      fs.Close();
                      theStore.DeleteFile(zTempFilename);
...
put32bits( zBuf, 0, d.Millisecond );
put32bits( zBuf, 4, d.Second );
put32bits( zBuf, 8, (int)d.Ticks );
put32bits( zBuf, 12, d.Hour );
;// memcpy(&zBuf[n], x, sizeof(x))
n += 16;// sizeof(x);

#else
SYSTEMTIME SYSTEMTIME = new _SYSTEMTIME();
...
put32bits( zBuf, n, i );//(memcpy(&zBuf[n], pid, sizeof(pid));
n += 4;// sizeof(pid);
}

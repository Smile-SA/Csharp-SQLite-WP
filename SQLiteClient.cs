...
namespace SQLiteClient
{
    ...
    public class SQLiteConnection : IDisposable
    {
        ...
        public void Dispose()
        {
            if (_open)
            {
                ...
                // http://stackoverflow.com/questions/5948499/methodaccessexception-when-updating-a-record-in-sqlite-db
    ...
    public class SQLiteCommand
    {
      ...
      "yyyyMMdd"
        ...
        public IEnumerable<Dictionary<string, object>> ExecuteSql()
        {
            var stmt = Prepare();

            int nbCols = Sqlite3.sqlite3_column_count(stmt);
            string[] colNames = new string[nbCols];
            for (int i = 0; i < nbCols; i++)
            {
                colNames[i] = Sqlite3.sqlite3_column_name(stmt, i);
            }
            while (Sqlite3.sqlite3_step(stmt) == Sqlite3.SQLITE_ROW)
            {
                ...
                for (int i = 0; i < nbCols; i++)
                {
                    if (colNames[i] == null)
                        continue;
                    var val = ReadCol(stmt, i, typeof(String));
